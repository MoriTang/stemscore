...
mod desktop_app {
    ...
    use crate::{
        audio::{LiveSession, SharedStatus, StemWaveforms},
        file_separator, model, model_store,
    };
    ...
    fn live_model_path() -> Result<PathBuf, String> {
        model_store::default_model_path().map_err(|error| format!("{error:#}"))
    }
